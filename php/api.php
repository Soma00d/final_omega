...
$saveLogPretest = function ($connexion) {
    ...
    $global_name_rapport = $_POST['globalNameRapport'];
    $model_name_rapport = $_POST['modelNameRapport'];
    
    
    $stmt = $connexion->prepare("INSERT INTO global_log (json_log, json_calib_log, json_testhw_log, json_powertest_log, part_number, serial_number, user_sso, role, type, node_id, fw_fct_version, fw_calib_version, sw_version, alim_tsui, alim_testbench, enable_tens, enable_freq,safety_tens, safety_freq, date, is_SRTL, should_have_SRTL, initial_safety_SRTL, initial_enable_SRTL, global_name_rapport, model_name_rapport) VALUES (:jsonlog, :jsoncaliblog, :jsontesthwlog, :jsonpowertestLog, :partnumber, :serialnumber, :user_sso, :role, :type, :nodeid, :fwfctv, :fwcalibv, :swv, :alimtsui, :alimtestbench, :enabletens, :enablefreq, :safetytens, :safetyfreq, NOW(), :issrtl,:shouldhavesrtl,:initialsafetysrtl,:initialenablesrtl, :globalnamerapport, :modelnamerapport)");
    $stmt->bindParam(':jsonlog', $jsonlog);
    $stmt->bindParam(':jsoncaliblog', $jsoncaliblog);
    $stmt->bindParam(':jsontesthwlog', $jsonTesthwlog);
    $stmt->bindParam(':jsonpowertestLog', $jsonPowerTestLog);
    ...
    $stmt->bindParam(':globalnamerapport', $global_name_rapport);
    $stmt->bindParam(':modelnamerapport', $model_name_rapport);
...
// Modify a dictionary (delete and re create)
$modifyDictionary = function ($connexion) {
    ...
    //$insertDescriptionDico = $connexion->query("UPDATE `admin_dictionary` SET `description`=$description,`family_name`=$refFamily,`model_name`=$refModel,`type_name`=$refType,`date`= NOW() WHERE family_id = $newID");
    
    $deleteDico =  $connexion->query("DELETE FROM `dictionaries` WHERE family_id = $newID");
    ...
    $insertUpdatedDico = $connexion->query($globalReq);
...
if (isset($_GET["function"])) {
    switch ($function) {
        ...
        case "save_new_dictionary":
            ...
            break;
        case "modify_dictionary":
            echo $modifyDictionary($connexion);
            break;
        default:
            echo "no param";
    }
