$(document).ready(function (){
   ...
   //OMEGA VALUES
    var currLongEnable = 0;
    var currTBLtopPan = 0;
    var currSc1LatDR = 0;
    var currTBLtopUpDown = 0;
    var currGlobGantry2 = 0;
    var currGlobTable = 0;
    var currFRTLlatGantry = 0;
    
    var currGlobv = 0;
    var currTsuiSupply = 0;
    var currFRTLgantry = 0;
    var currUnreg5 = 0;
    var currUnreg12 = 0;
    
    var initial_globv = 0;
    var initial_tsui_supply = 0;
    var initial_frtl_gantry = 0;
    var initial_unreg5 = 0;
    var initial_unreg12 = 0;
    
    // OMEGA déclaration des variables current_ qui sont actualisées à chaque tick de boucle provenant du PIC
    var curr_TBL_TOP_PAN;
    var curr_GLOBAL_TABLE_ENABLE;
    var curr_GLOBAL_GANTRY_ENABLE;
    var curr_FRTL_LAT_GANTRY_EN;
    var curr_LONG_ENABLE;
    var curr_TBL_TOP_UP_DOWN_EN;
    var curr_SC1_LAT_DR_REQ;
    var curr_TBL_DRIVE_DOWN;
    var curr_TBL_DRIVE_UP;
    
    var curr_LATERAL_SWA1;
    var curr_SCI_LAT_DR_REQ;
    var curr_AUTO_POS_DR_REQ;
    ...
    //-----------------------------------------------------//
    // les _MODE permettent de trier les interactions et les traitements à effectuer lors de chaque message reçu en provenance de webSocket
    var _MODE = "START";
                        ...
                        $.ajax({
                            ...
                            success: function (data, statut) {
                                if (data.length == 0) {
                                    ...
                                    if(globalName == "OMEGA"){
                                        $("#content_home .information_finaltest .test_bt_home.omega").removeClass("hidden");
                                        $("#content_home .information_finaltest .test_bt_home.elegance").addClass("hidden");
                                    }else{
                                        $("#content_home .information_finaltest .test_bt_home.elegance").removeClass("hidden");
                                        $("#content_home .information_finaltest .test_bt_home.omega").addClass("hidden");
                                    }
                                    $("#content_home .commentary_bloc").removeClass("hidden");
                                    $(".head_userinfo").removeClass("hidden");
                                    $(".head_userinfo .info .role_user").html("Repair");
                                                    ...
                                                    joystickVerifyContainer.append("<div class='realtime_joysticks_val id" + data[iter].id + "' data-symb='" + data[iter].symbol_name + "' data-standard='" + data[iter].standard_name + "' data-minaxis='" + data[iter].threshold_min_axis + "' data-maxaxis='" + data[iter].threshold_max_axis + "' data-minzero='" + data[iter].threshold_min_zero + "' data-maxzero='" + data[iter].threshold_max_zero + "'>"
                                                            ...
                                                            + "<div class='bloc_raw_data'>"
                                                               ...
                                                               + "<span class='text_config'>Zero X : </span><span class='raw_zero_x get_val'  data-descri='zero_x'>-</span><br>"
                                                                ...
                                                                + "<div class='bloc_right_joy'>"
                                                                + "<span class='text_config'>Zero Y : </span><span class='raw_zero_y get_val' data-descri='zero_y'>-</span><br>"
                                                                + "<span class='text_config'>Left : </span><span class='raw_min_y get_val' data-descri='bottom'>-</span><br>"
                                                                + "<span class='text_config'>Right : </span><span class='raw_max_y get_val' data-descri='top'>-</span>"
                                                                + "</div>"                                                            
                                                            + "</div>"
                                                            + "</div>"
                                                            + "</div>");
            ...
            case "TESTFINAL": // le mode pour la section test final de la repair
                ...
                if (message.type == "from_GW") {
                    ...
                    if (waitingAction != "") {
                        switch (waitingAction) {
                            ...
                            case "ENABLE":
                                if (waitingPressValue == canData) {
                                    if (waitingEnableAndSafety == 1) {                                        
                                        setTimeout(function(){
                                            setEnableAndSafetyValues("enable", canData);
                                        }, 50)                                        
                                    }
                                    pressValueContinue = 1;
                                }
                                if (waitingReleaseValue == canData) { 
                                    releaseValueContinue = 1;
                                    setTimeout(function(){
                                        setEnableAndSafetyValuesRel("enable", canData);                                         
                                    }, 500)    
                            ...
                            case "JOYSTICK_X_LEFT":
                                console.log("on rentre dans la case joystick_x_left");
                                if (waitingID == canId) {
                                    ...
                                    console.log("valeur reçue et analyzed :"+instant_value_joy)
                            ...
                            case "JOYSTICK_X_RIGHT":
                                if (waitingID == canId) {
                                    ...
                                    var instant_value_joy = canData.substring(oct1, oct1 + 2);
                                    instant_value_joy = convertHexa(instant_value_joy);
                                    console.log("valeur reçue et analysed :"+instant_value_joy)
                                    if (instant_value_joy > 0) {
                                        if (instant_value_joy >= last_value_joy) {
                                            console.log("last : " + last_value_joy, "instant : " + instant_value_joy);
                                            last_value_joy = instant_value_joy;
                    ...
                    console.log("val tens" + currEnableT + " " + currEnableF + " " + currGlobalVoltage + " " + currTsuiVoltage)
                        ...
                        currGlobv = globv;
                        currTsuiSupply = tsuiSupply;
                        currFRTLgantry = FRTLgantry;
                        currUnreg5 = unreg5;
                        currUnreg12 = unreg12;
                        ...
                        currSc1LatDR = sc1LatDR;
                        currTBLtopUpDown = TBLtopUpDown;
                        currGlobGantry2 = globGantry2;
                        currGlobTable = globTable;
                        currFRTLlatGantry = FRTLlatGantry;
            ...
            case "TEST_HARDWARE_OMEGA": // le mode utilisé pour toutes les opérations de la page Test hardware Omega dans la section repair test final
                ...
                if (message.type == "from_pic") {
                    ...
                    if(message.typeMsg == "E"){
                        ...
                        curr_TBL_TOP_PAN = TBLtopPan;
                        curr_GLOBAL_TABLE_ENABLE = globTable;
                        curr_GLOBAL_GANTRY_ENABLE = globGantry2;
                        curr_FRTL_LAT_GANTRY_EN = FRTLlatGantry;
                        curr_LONG_ENABLE = longEnable;
                        curr_TBL_TOP_UP_DOWN_EN = TBLtopUpDown;
                        curr_SC1_LAT_DR_REQ = sc1LatDR;
                        curr_TBL_DRIVE_DOWN = tblDriveDown;
                        curr_TBL_DRIVE_UP = tblDriveUp;
    ...
    //Confirmation de mise à jour d'un commentaire associé à un Serial Number + lancement de la fonction associée
    ...
    //Recupération Ajax des serial number et check si il existe dans la BDD
    ...
    //Ajout d'un nouveau serial number dans la BDD
    ...
    //Mise à jour d'un commentgaire lié au SN dans la BDD
    ...
    function setInitialDisplayByModel(globalName, modelName, typeChoice){        
        ...
        $(".continue_to_final").addClass("hidden");
        $(".continue_to_testhw").addClass("hidden");
        ...
        $(".test_hw_container .smarthandle_omega").addClass("hidden");
        $(".test_hw_container .tssc_omega").addClass("hidden");
        ...
        if (globalName == "OMEGA") {       
            ...
            $(".warning_firmware.elegance").addClass("hidden");
            $(".warning_firmware.omega").removeClass("hidden");
            $(".eprom_protect").addClass("hidden");
            ...
            if(modelName == "TSSC" ){
                ...
                $(".warning_firmware.omega").addClass("hidden");
                $(".test_hw_container .tssc_omega").removeClass("hidden");
                ...
                if(modelName == "SMARTBOX"){
                    $(".test_hw_container .smartbox_omega").removeClass("hidden");
                    $(".test_hw_container .smarthandle_omega").addClass("hidden");
                }else{
                    $(".test_hw_container .smartbox_omega").addClass("hidden");
    ...
    function displayFinalTest(indexFinal) {
        ...
        setTimeout(function () {
            ...
            }else if(isSafety == 1  && globalName == "ELEGANCE"){
        ...
        if(globalName == "ELEGANCE"){
            ...
            var can_id = finalTestEntriesTest[indexFinal]["can_id"];
        }
        ...
        currSignalStart = postSignal + dlcSign + currOnSignal;
        currSignalStop = postSignal + dlcSign + currOffSignal;
    ...
    function nextStepFinal(result) {
        ...
        if (indexFinal < maxIndexFinal) {
            if(isSafety == 1 && globalName == "ELEGANCE"){
                $("#testfinal_container").addClass("hidden");
    ...
    function setEnableAndSafetyValues(type, canVal) {
        if(type== "enable"){
            if(globalName == "ELEGANCE"){
                enableF = currEnableF;
                enableT = currEnableT;
    ...
    function setEnableAndSafetyValuesRel(type, canVal) {
        if(type== "enable"){
    ...
    $(".switch_nb.omega span").on('click', function(){
        if($(this).hasClass("2pos")){
            $(this).addClass("selected");
            $(".switch_nb.omega .3pos").removeClass("selected");
        }else{
            $(this).addClass("selected");
            $(".switch_nb.omega .2pos").removeClass("selected");
        }
    })
    ...
    $(".get_switch").on('click', function () {
        if(globalName == "OMEGA"){
            ...
            if($(".switch_nb.omega .2pos").hasClass("selected")){
                posNb = 2;
            }else if($(".switch_nb.omega .3pos").hasClass("selected")){
                posNb = 3;
            }
            _MODE = "CALIBRATION";
            
            var startTestMode = Cal_post + "030000" + "1fc42f00" + "060000"
            sendSignal(startTestMode);

            if(posNb == 2){
                var ping2pos = Cal_post+"030000"+ "1fc42f00" +"000000";
                setTimeout(function(){
                    waitPingResponse = "1fc00f51";
                    sendSignal(ping2pos);                        
                    setTimeout(function(){
                        var responseFinal = finalResponseData.substring(6,8);

                        switch(responseFinal){                                
                            case "01":                                        
                                resultDone = "right";                                        
                                break;
                            case "00":                                        
                        ...
                        $(".position_result span").html(resultDone);

                    },200)
                },200)
            }else{
                var ping3pos = Cal_post+"030000"+ "1fc42f00" +"000015"
                var ping3posBis = Cal_post+"030000"+ "1fc42f00" +"000016"
                setTimeout(function(){
                    waitPingResponse = "1fc00f51";
                    sendSignal(ping3pos);                        
                    setTimeout(function(){
                        var response = finalResponseData.substring(6,8);

                        waitPingResponse = "1fc00f51";
                        sendSignal(ping3posBis);

                        setTimeout(function(){
                            var response2 = finalResponseData.substring(6,8);
                            var responseFinal = response + response2
    ...
    function calibrateZeroLong(subindexX, subindexY, identifier) {
        if(globalName =="OMEGA"){
            ...
            $(".bloc_calibrate.id" + identifier).find(".status_calib").html("Set ZERO position");
            $(".bloc_calibrate.id" + identifier).find(".action_calib").html("<img src='images/zero_arrow.png'>");
            if(modelName == "TSSC"){var canIdnew = "1fc22f00"}else( canIdnew = "1fc42f00");
            
            $(".bloc_calibrate.id" + identifier).find(".validate_calib").on('click', function () {
                var x1 = Cal_post + "030000" + canIdnew + "00" + subindexX;
                sendSignal(x1);
                if(modelName == "TSSC"){waitPingResponse = "1fc00f41"}else( waitPingResponse = "1fc00f51");
                setTimeout(function () {
                    var x1Response = finalResponseData.substring(6,8);                    
                    var x2 = Cal_post + "030000" + canIdnew + "000" + addHexValOmega(subindexX, "1");                    
                    sendSignal(x2);
                    if(modelName == "TSSC"){waitPingResponse = "1fc00f41"}else( waitPingResponse = "1fc00f51");
                    setTimeout(function () {
                        ...
                        if (seuilMinZero <= afinal && afinal < seuilMaxZero && modeEngineering == 0) {
                            ...
                            var writeResult = Cal_post + "050000" + canIdnew + "040" + addHexValOmega(subindexX, "c") + result;
    ...
    function calibrateMinLat(subindexX, subindexY, identifier) {
        if(globalName =="OMEGA"){
            ...
            $(".bloc_calibrate.id" + identifier).find(".validate_calib").on('click', function () {
                var y1 = Cal_post + "030000" + canIdnew + "00" + subindexY;
                sendSignal(y1);
                if(modelName == "TSSC"){waitPingResponse = "1fc00f41"}else( waitPingResponse = "1fc00f51");
                setTimeout(function () {
                    var y1Response = finalResponseData.substring(6,8);                    
                    var y2 = Cal_post + "030000" + canIdnew + "000" + addHexValOmega(subindexY, "1");                    
                    sendSignal(y2);
                    if(modelName == "TSSC"){waitPingResponse = "1fc00f41"}else( waitPingResponse = "1fc00f51");
                    setTimeout(function () {
                ...
                setTimeout(function () {
                    ...
                    setTimeout(function () {
                        ...
                        if (seuilMinZero <= afinal && afinal < seuilMaxZero && modeEngineering == 0) {
                            ...
                            var writeResult = Cal_post + "050000" + canIdnew + "040" + addHexValOmega(subindexY, "14") + result;
                            sendSignal(writeResult);
                            resetCalibration(identifier);
                        }else if(modeEngineering == 1){
                            $(".realtime_joysticks_val.id" + identifier).find(".raw_zero_y").html(afinalSigned);
    ...
    function resetCalibration(identifier) {
        ...
        $(".bloc_calibrate.id" + identifier).find(".validate_calib").on('click', function () {
            ...
            setTimeout(function () {
                ...
                if (count == count2) {
                    if(globalName == "OMEGA"){
                        $(".continue_to_testhw").removeClass("hidden");
                    }else{
    ...
    function fillSwitchTest(switchNb){
        ...
        switch(switchNb){
            case "2":
                containerSwitch.append(
                    "<div class='switch_line_test 2pos' data-position='left'>"+
                    ...
                    "</div>"+
                    "<div class='switch_line_test 2pos' data-position='right'>"+
            ...
            case "3":
                containerSwitch.append(
                    "<div class='switch_line_test 3pos' data-position='left'>"+
                        "<div class='switch_name'>Position LEFT</div>"+
                        "<button class='switch_bt_check'>Check Position</button>"+
                        "<div class='switch_test'></div>"+
                        "<div class='switch_result'></div>"+
                    "</div>"+
                    "<div class='switch_line_test 3pos' data-position='middle'>"+
                        "<div class='switch_name'>Position MIDDLE</div>"+
                        "<button class='switch_bt_check'>Check Position</button>"+
                        "<div class='switch_test'></div>"+
                        "<div class='switch_result'></div>"+
                    "</div>"+
                    "<div class='switch_line_test 3pos' data-position='right'>"+
                        "<div class='switch_name'>Position RIGHT</div>"+
                        "<button class='switch_bt_check'>Check Position</button>"+
                        "<div class='switch_test'></div>"+
                        "<div class='switch_result'></div>"+
            ...
            case "4":
                containerSwitch.append(
                    "<div class='switch_line_test 4pos' data-position='left'>"+
                        "<div class='switch_name'>Position LEFT</div>"+
                        "<button class='switch_bt_check'>Check Position</button>"+
                        "<div class='switch_test'></div>"+
                        "<div class='switch_result'></div>"+
                    "</div>"+
                    "<div class='switch_line_test 4pos' data-position='front'>"+
                        "<div class='switch_name'>Position FRONT</div>"+
                        "<button class='switch_bt_check'>Check Position</button>"+
                        "<div class='switch_test'></div>"+
                        "<div class='switch_result'></div>"+
                    "</div>"+
                    "<div class='switch_line_test 4pos' data-position='right'>"+
                        "<div class='switch_name'>Position RIGHT</div>"+
                        "<button class='switch_bt_check'>Check Position</button>"+
                        "<div class='switch_test'></div>"+
                        "<div class='switch_result'></div>"+
                    "</div>"+
                    "<div class='switch_line_test 4pos' data-position='back'>"+
        ...
        $(".switch_bt_check").on('click', function(){
            if(globalName == "OMEGA"){
                ...
                }else if($(this).parent().hasClass("3pos")){
                    var ping3pos = Cal_post+"030000"+ "1fc42f00" +"000015"
                    var ping3posBis = Cal_post+"030000"+ "1fc42f00" +"000016"
                    setTimeout(function(){
                        waitPingResponse = "1fc00f51";
    ...
    function updateVerifyData(verifyVal, axis, currentIdentifier) {
        ...
        if (axis == "x") {
            if(globalName == "OMEGA"){
                var valueFinal = convertHexa2(verifyVal);
            }else{
    ...
    function pingGetAxisValue(subindexX, subindexY, identifier) {
        if (subindexX !== "null") {
            if(globalName =="OMEGA"){
                if(modelName =="TSSC"){
                    var axisRawXverif = Cal_post + "030000" + "1fc22f00" + "010" + addHexValOmega(subindexX, "4");
                    sendSignal(axisRawXverif);
                }else{
                    var axisRawXverif = Cal_post + "030000" + "1fc42f00" + "010" + addHexValOmega(subindexX, "4");
                    sendSignal(axisRawXverif);
                }
            }else{
        ...
        if (subindexY !== "null") {
            if(globalName =="OMEGA"){
                if(modelName =="TSSC"){
                    var axisRawXverif = Cal_post + "030000" + "1fc22f00" + "010" + addHexValOmega(subindexY, "4");
                    sendSignal(axisRawXverif);
                }else{
                    var axisRawXverif = Cal_post + "030000" + "1fc42f00" + "010" + addHexValOmega(subindexY, "4");
                    sendSignal(axisRawXverif);
                }
            }else{
    ...
    $(".continue_to_testhw").on('click', function () {       
        FWcalibV = $(".sw_config").html();
        $(".verify_calibration").removeClass("hidden");
        $(".stop_calibration_verif").addClass("hidden");
        clearInterval(intervalVerify);
    ...
    $(".continue_to_final").on('click', function () {
        if(modelName == "TSSC"){
            var stopTestMode = Cal_post + "030000" + "1fc22f00" + "070000"
            sendSignal(stopTestMode);
        }else{
    ...
    function getTestHwLog(){
        ...
        testhwLogJSON = JSON.stringify(testhwLog);
        console.log(testhwLogJSON);
    }    
...
//         }
    ...
    $(".test_hw_container .bt_check.start").on('click', function(){
        ...
        var hw2 = $(this).parent().data('hw2');
        var hw3 = $(this).parent().data('hw3');
        ...
        _this.parent().find(".bt_check.stop").removeClass("hidden");
        launchTestProcessHW(hw1, hw2, hw3, ping, _this);
    });
    
    $(".test_hw_container .bt_check.start_master_pin").on('click', function(){
       var _this = $(this)
       var ping = $(this).parent().data('ping');
       _MODE = "TEST_HARDWARE_OMEGA";
       //met la pin en master
       sendSignalPic("DC")
       ...
       setTimeout(function(){
           ...
           if(response.substring(0,2)== "80" && response.substring(6,8)== "01"){
               console.log("on a reçu 01 après le test")
               sendSignalPic("D@");
               ...
               setTimeout(function(){
                   ...
                   setTimeout(function(){
                       var response2 = finalResponseData;
                       if(response2.substring(0,2)== "80" && response2.substring(6,8)== "00"){
                           console.log("on a reçu 00 après le test")
    ...
    $(".test_hw_container .bt_check.stop").on('click', function(){
        ...
        var ping = $(this).parent().data('ping');
        _this.addClass("hidden");
        _this.parents(".line_table_test").find(".user_action").html("-");
        $(".test_hw_container .bt_check.start").each(function(){
            $(this).removeClass("hidden");
        });
       ...
       if(globalName == "OMEGA" && (modelName == "SMARTBOX" || modelName == "SMARTHANDLE")){
            sendSignalPic("9");
            if(actualLine.hasClass("logical")){
    ...
    function launchTestProcessHW(hw1, hw2, hw3, ping, _this){
        _MODE = "TEST_HARDWARE_OMEGA";
        console.log(hw1+" "+hw2+" "+hw3+" "+ping);
        var actualLine = _this.parents(".line_table_test");
        actualLine.find(".user_action").html("Press and hold the button");
        
        if(globalName == "OMEGA" && (modelName == "SMARTBOX" || modelName == "SMARTHANDLE")){
            var signalPing = Cal_post+"0300001fc42f0000"+ping;
            sendSignalPic("8");            
            if(actualLine.hasClass("logical")){
                sendSignal(Cal_post+"0400001fc42f0003010501");
                sendSignal(Cal_post+"0400001fc42f0003010601");
            }
        }else{
            sendSignalPic("6");
        ...
        intervalTestHW = setInterval(function(){
           ...
           setTimeout(function(){
               ...
               if(response.substring(0,2)== "80" && response.substring(6,8)== "01"){
                   console.log(response);
                   console.log("button is pressed : "+response.substring(6,8));
                   checkHWvaluesPress(hw1, hw2, hw3, ping, actualLine);
    ...
    function checkHWvaluesPress(hw1, hw2, hw3, ping, _this){
        clearInterval(intervalTestHW);
        var error = 0;
        if(hw1 != "" && hw1 && hw1!= "-"){
            if(checkHWsignal(hw1, "press", _this) == 0){
                error++;
                _this.find(".hw_signals .hw1").addClass("error");
                _this.find(".hw_signals .hw1").removeClass("valide");
            }else{
                _this.find(".hw_signals .hw1").removeClass("error");
    ...
    function checkHWsignal(signalHW, state, _this){                
        ...
        switch(signalHW){
            case "TBL_TOP_PAN":
                console.log("TBL_TOP_PAN -> "+curr_TBL_TOP_PAN);
                
                if(curr_TBL_TOP_PAN > 20 && curr_TBL_TOP_PAN < 25 && state == "press"){
                   result = 1; 
                   _this.find(".hw_signals .tbl_top_pan .values").html("");
                   _this.find(".hw_signals .tbl_top_pan .values").append(curr_TBL_TOP_PAN.toFixed(2)+"V")
                }else if(curr_TBL_TOP_PAN < 2 && state == "release"){
                   result = 1;
                   _this.find(".hw_signals .tbl_top_pan .values").append(" / 0V")
                }
                else{
                    result = 0; 
                }
                break;
            case "GLOBAL_TABLE_ENABLE":
                console.log("GLOBAL_TABLE_ENABLE -> "+curr_GLOBAL_TABLE_ENABLE);
                
                if(curr_GLOBAL_TABLE_ENABLE > 20 && curr_GLOBAL_TABLE_ENABLE < 25){
                    result = 1; 
                    _this.find(".hw_signals .global_table_enable .values").html("");
                    _this.find(".hw_signals .global_table_enable .values").append(" "+curr_GLOBAL_TABLE_ENABLE.toFixed(2)+"V")
                }else if(curr_GLOBAL_TABLE_ENABLE < 2 && state == "release"){
                    result = 1;
                    _this.find(".hw_signals .global_table_enable .values").append("  / 0V")
                }
                else{
                    result = 0; 
                }
            ...
            case "GLOBAL_GANTRY_ENABLE":
                ...
                break;
            case "FRTL_LAT_GANTRY_EN":
                console.log("FRTL_LAT_GANTRY_EN -> "+curr_FRTL_LAT_GANTRY_EN);
                
                if(curr_FRTL_LAT_GANTRY_EN > 20 && curr_FRTL_LAT_GANTRY_EN < 25){
                   result = 1; 
                   _this.find(".hw_signals .frtl_lat_gantry_en .values").html("");
                   _this.find(".hw_signals .frtl_lat_gantry_en .values").append(" "+curr_FRTL_LAT_GANTRY_EN.toFixed(2)+"V")
                }else if(curr_FRTL_LAT_GANTRY_EN < 2 && state == "release"){
                   result = 1; 
                   _this.find(".hw_signals .frtl_lat_gantry_en .values").append(" / 0V")
                }
                else{
                    result = 0; 
                }
                break;
            case "LONG_ENABLE":
                console.log("LONG_ENABLE -> "+curr_LONG_ENABLE);
                
                if(curr_LONG_ENABLE > 20 && curr_LONG_ENABLE < 25){
                   result = 1; 
                   _this.find(".hw_signals .long_enable .values").html("");
                   _this.find(".hw_signals .long_enable .values").append(" "+curr_LONG_ENABLE.toFixed(2)+"V")
                }else if(curr_LONG_ENABLE < 2 && state == "release"){
                   result = 1; 
                ...
                break;
            case "TBL_TOP_UP_DOWN_EN":
                
                console.log("TBL_TOP_UP_DOWN_EN -> "+curr_TBL_TOP_UP_DOWN_EN);
                if(curr_TBL_TOP_UP_DOWN_EN > 20 && curr_TBL_TOP_UP_DOWN_EN < 25){
                   result = 1; 
                   _this.find(".hw_signals .tbl_top_up_down_en .values").html("");
                   _this.find(".hw_signals .tbl_top_up_down_en .values").append(" "+curr_TBL_TOP_UP_DOWN_EN.toFixed(2)+"V")
                }else if(curr_TBL_TOP_UP_DOWN_EN < 2 && state == "release"){
                   result = 1; 
                   _this.find(".hw_signals .tbl_top_up_down_en .values").append(" / 0V")
                }
                else{
                    result = 0; 
                }
                break;
            case "SC1_LAT_DR_REQ":
                console.log("SC1_LAT_DR_REQ -> "+curr_SC1_LAT_DR_REQ);
                
                if(curr_SC1_LAT_DR_REQ > 20 && curr_SC1_LAT_DR_REQ < 25){
                   result = 1; 
                   _this.find(".hw_signals .sc1_lat_dr_req .values").html("");
                   _this.find(".hw_signals .sc1_lat_dr_req .values").append(" "+curr_SC1_LAT_DR_REQ.toFixed(2)+"V")
                }else if(curr_SC1_LAT_DR_REQ < 2 && state == "release"){
                   result = 1; 
                   _this.find(".hw_signals .sc1_lat_dr_req .values").append(" / 0V")
                }
                else{
                    result = 0; 
                }
                break;
            case "TBL_DRIVE_DOWN":
                console.log("TBL_DRIVE_DOWN -> "+curr_TBL_DRIVE_DOWN);
                if(curr_TBL_DRIVE_DOWN == 1){
                   result = 1; 
                   _this.find(".hw_signals .tbl_drive_down .values").html("");
                   _this.find(".hw_signals .tbl_drive_down .values").append("<span class='val_hw'>"+curr_TBL_DRIVE_DOWN+"</span>")
                }else if(curr_TBL_DRIVE_DOWN == 0 && state == "release"){
                   result = 1; 
                   _this.find(".hw_signals .tbl_drive_down .values").append(" / "+curr_TBL_DRIVE_DOWN)
                }
                else{
                    result = 0; 
                }
                break;
            case "TBL_DRIVE_UP":
                console.log("TBL_DRIVE_UP -> "+curr_TBL_DRIVE_UP);
                
                if(curr_TBL_DRIVE_UP == 1){
                   result = 1; 
                   _this.find(".hw_signals .tbl_drive_up .values").html("");
                   _this.find(".hw_signals .tbl_drive_up .values").append("<span class='val_hw'>"+curr_TBL_DRIVE_UP+"</span>")
                }else if(curr_TBL_DRIVE_UP == 0 && state == "release"){
                   result = 1; 
                   _this.find(".hw_signals .tbl_drive_up .values").append(" / "+curr_TBL_DRIVE_UP)
                }
                else{
                    result = 0; 
                }
                break;
            case "LATERAL_SWA1":
                console.log("LATERAL_SWA1 -> "+curr_LATERAL_SWA1);
            ...
            case "GLOBAL_GANTRY_ENABLE":
                ...
                break;
            case "SCI_LAT_DR_REQ":
                console.log("SCI_LAT_DR_REQ -> "+curr_SCI_LAT_DR_REQ);
                ...
                break;
            case "AUTO_POS_DR_REQ":
                console.log("AUTO_POS_DR_REQ -> "+curr_AUTO_POS_DR_REQ);
               ...
               if(curr_AUTO_POS_DR_REQ > 20 && curr_AUTO_POS_DR_REQ < 25){
                ...
                break;
            case "SCI_FRTL_DR_REQ":
                console.log("SCI_FRTL_DR_REQ -> "+curr_SCI_FRTL_DR_REQ);
    ...
    $(".head_logo").on('click', function(){
        ...
        if($(this).hasClass("repair_mode") || $(this).hasClass("manufacturing_mode")){
            ...
            if(globalName == "OMEGA" && modelName =="TSSC"){
                ...
                if($(".switch_mast").hasClass("activated")){
                    sendSignal("002400806d68d7551407f09b861e3aad000549a8440800001fc20f000e00000000000000");                    
                    $(".switch_mast").removeClass("activated");
                    $(".switch_mast").find("img").attr("src", "images/switch_off.png");
                }    
            }else if(globalName == "OMEGA" && modelName =="SMARTBOX"){
                sendSignalPic("9");        
                sendSignalPic("D`");
...
//                sendSignal("002400806d68d7551407f09b861e3aad000549a844050000066222400000000A00000000");
    ...
    function minusHexValOmega(c1, c2) {
        var hexStr = (parseInt(c1, 16) - parseInt(c2, 16)).toString(16);
    ...
    function decimalToHexStringCalib(number){
        if (number < 0){
            number = 0xFFFF + number + 1;
        }else if(number > 0 && number < 16){
            number = "000"+number.toString(16);
        }else{
            number = "00"+number.toString(16);
        }
